---
title: "Globe Head"
description: "Normal enemy with 148 (Asc 158) HP. Shocking Slap -> Channel Lightning -> Galvanic Burst -> repeat"
---

# Globe Head

<div class="wiki-entity-hero" markdown="1">
<a class="wiki-image-link" href="/media/enemies/globe-head.webp" target="_blank" rel="noopener" aria-label="View full image: Globe Head" title="View full image: Globe Head"><img class="wiki-image wiki-image--enemy-detail" src="/media/enemies/globe-head.webp" alt="Globe Head" loading="lazy" decoding="async"></a>
<div class="wiki-entity-hero__details" markdown="1">
| Property | Value |
|---|---|
| Type | Normal |
| HP | 148 (Asc 158) |
| Pattern | Shocking Slap -> Channel Lightning -> Galvanic Burst -> repeat |
</div>
</div>

## Moves

| Move | Intent | Damage | Block | Heal |
|---|---|---|---|---|
| Channel Lightning | Attack | 6x3 (Asc 7x3) damage | - | - |
| Shocking Slap | Attack + Debuff | 13 (Asc 14) damage | - | - |
| Galvanic Burst | Attack + Buff | 16 (Asc 17) damage | - | - |

## Encounters

| Encounter | Room | Act |
|---|---|---|
| A Lone Globe Head | Monster | Act 3 - Glory |

[Back to all enemies](../enemies.md)
