---
title: "Thieving Hopper"
description: "Normal enemy with 79 (Asc 84) HP. Thievery -> Flutter -> Hat Trick -> Nab -> Escape -> repeat"
---

# Thieving Hopper

<div class="wiki-entity-hero" markdown="1">
<a class="wiki-image-link" href="/media/enemies/thieving-hopper.webp" target="_blank" rel="noopener" aria-label="View full image: Thieving Hopper" title="View full image: Thieving Hopper"><img class="wiki-image wiki-image--enemy-detail" src="/media/enemies/thieving-hopper.webp" alt="Thieving Hopper" loading="lazy" decoding="async"></a>
<div class="wiki-entity-hero__details" markdown="1">
| Property | Value |
|---|---|
| Type | Normal |
| HP | 79 (Asc 84) |
| Pattern | Thievery -> Flutter -> Hat Trick -> Nab -> Escape -> repeat |
</div>
</div>

## Moves

| Move | Intent | Damage | Block | Heal |
|---|---|---|---|---|
| Thievery | Attack + Debuff | 17 (Asc 19) damage | - | - |
| Nab | Attack | 14 (Asc 16) damage | - | - |
| Hat Trick | Attack | 21 (Asc 23) damage | - | - |
| Flutter | Buff | - | - | - |
| Escape | Escape | - | - | - |

## Encounters

| Encounter | Room | Act |
|---|---|---|
| Thieving Hopper | Monster | Act 2 - Hive |

[Back to all enemies](../enemies.md)
